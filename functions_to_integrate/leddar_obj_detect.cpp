...
int detect_wall(std::vector<float> v, float measure_error, float flat_error) {
  ...
  b = r * (sdy/sdx);
  a = my - mx;
  // std::cout << std::to_string(b) << " " << a << std::endl;
  ...
  bool wall = true;
  for ( unsigned int i=0; i<v.size(); i++ ){
    if ( abs( (b*int(i)+a) - v.at(i) ) > measure_error )  {
      std::cout << (b*int(i)+a) - v.at(i) << endl;
      wall = false; 
    }
  }
...
{ 
  ...
  int wall = detect_wall(vec, .1, .1);
  std::cout << std::to_string(wall) << std::endl;
